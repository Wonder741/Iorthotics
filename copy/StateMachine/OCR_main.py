    ...
    try:
        while True:
            ...
            if data_received == 'wait pose':
                if Coordinate_empty_flag:
                    image = GUI_function.camera_capture(int(ob_camera_index_var.get()), int(camera_resolution_width_var.get()), int(camera_resolution_height_var.get()), 255)
                    image_file_name = GUI_function.image_save(image, OB_image_store_path)
                    # Return a list of coordinate list from google cloud object detection
                    coordinate_list = ob_fuction(OB_image_store_path + '\\scan.jpg')
                    if len(coordinate_list) > 0:
                        conn.send(str.encode('go pick'))
                        log_message('Send message to robot: go pick')
                    else:
                        conn.send(str.encode('go finish'))
                        log_message('Finish task of robot')

                
                # Check if the list is not empty before proceeding
                if coordinate_list:
                    # Assign the first list to floats_to_send
                    floats_to_send = coordinate_list[0]
                    # Remove the first element from the list
                    coordinate_list.pop(0)
                    # Check if the coordinate_list is now empty and set a flag accordingly
                    Coordinate_empty_flag = len(coordinate_list) == 0
                else:
                    # Handle the case where coordinate_list was initially empty
                    floats_to_send = None
                    Coordinate_empty_flag = True  # Setting the flag as True since the list is empty
                
                # Multiply each float by 1000 and convert to int
                processed_floats = [int(x * 1000) for x in floats_to_send]
                ...
                image = GUI_function.camera_capture(int(ocr_camera_index_var.get()), int(camera_resolution_width_var.get()), int(camera_resolution_height_var.get()), 255)
